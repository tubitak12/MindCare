...
class SoundsScreen extends StatelessWidget {
  ...
  final List<Map<String, dynamic>> categories = const [
    ...
    {'title': 'Meditasyon', 'icon': Icons.self_improvement, 'color': Colors.purple},
    {'title': 'Gece & Uyku', 'icon': Icons.nightlight_round, 'color': Colors.indigo},
      ...
      body: SafeArea(
        child: GridView.builder(
          ...
          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
            ...
            mainAxisSpacing: 15,
            crossAxisSpacing: 15,
            childAspectRatio: 1.0, // Kutuları tam kare yaparak 6 taneyi sığdırdık
          ),
                ...
                child: StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (soundDocs.isEmpty) {
                      return const Center(child: Text("Bu kategoride henüz ses yok."));
                    }

                    return ListView.builder(
                      itemCount: soundDocs.length,
                      itemBuilder: (context, index) {
                        final data = soundDocs[index].data() as Map<String, dynamic>;
                        bool isPlaying = playingTitle == data['name'];

                        return Container(
                          margin: const EdgeInsets.only(bottom: 10),
                          decoration: BoxDecoration(
                            color: isPlaying ? category['color'].withOpacity(0.06) : Colors.grey[50],
